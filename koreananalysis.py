...
with st.form("cognitive_form"):
    ...
    q2 = st.text_area(
        "2. 최근 내 기분이나 일상 패턴을 갑자기 흔들어 놓았던 계기나 사건이 있나요? 그리고 그 원인이 무엇이라고 생각하나요?", 
        height=90,
        placeholder="예: 지난주에 친구와 작은 오해가 생기는 바람에 마음이 많이 불편해졌다. 신경을 쓰다 보니 결국 잠도 제대로 못 잤다."
    )
    q3 = st.text_area(
        "3. 최근 있었던 일 중에서 '내가 그때 다르게 행동했다면 어땠을까' 하고 남는 아쉬움이 있었나요?", 
        height=90,
        placeholder="예: 그때 내가 조금만 더 참았으면 일이 이렇게 커지지 않았을 텐데 아쉽다. 내 고집 때문에 상황을 망친 것 같아 후회가 된다."
    )
    q4 = st.text_area(
        "4. 조만간 결과를 확인해야 하거나 해야 할 일를 앞두고 어떤 생각이 드나요?", 
        height=90,
    ...
    q5 = st.text_area(
        ...
        height=90,
        placeholder="예: 지난 과제를 끝내느라 밤을 새우는 바람에 지쳤다. 쉬고 난 뒤에도 여전히 머리가 멍하고 피곤함이 남아있다."
    )
